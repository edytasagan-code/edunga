...
export default function TrueFalseTableEditor({
  paragraphId,
  node,
  editorRoot,
  selectedNodeId,
...
function TrueFalseTableRowEditor({
  paragraphId,
  tableNodeId,
  rowId,
  row,
  ...
  const navigator = useMemo(
    ...
    [row.statement, paragraphId, onMathRemove]
  );

  return (
    <>
      {row.statement.map((child, index) => {
        if (child.type === "text") {
          const previous = row.statement[index - 1];
